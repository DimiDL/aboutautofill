...
test_destination_dir = os.path.join(
    destination_dir, "browser/extensions/formautofill/test/browser/heuristics/third_party"
)
    ...
    for file_name in os.listdir(page_dir):
        ...
        try:
            if os.path.isdir(file_path):
                ...
                print(f"Moved directory {file_name} to {fixture_dir}")
            elif os.path.isfile(file_path):
                shutil.copy2(file_path, target_path)
                print(f"Moved file {file_name} to {fixture_dir}")
        except IOError as e:
